...
import { crm, knowledgeBase, businessActivities, forecasts, goals, salesSkills, skillCalendar } from '../api/endpoints';
import { useApi } from '../hooks/useApi';
import { useWebSocket } from '../hooks/useWebSocket';
import { useSkillTaskTracker } from '../hooks/useSkillTaskTracker';
import { useToast } from '../context/ToastContext';
import type { Contact, Deal, KnowledgeBaseArticle, BusinessActivity, Forecast, Goal, SalesSkill, SalesWorkflow, SalesWorkflowRun, ExecutionPlan, SkillCalendarEntry, PlanStats } from '../types';

const TABS = ['Plan Calendar', 'Pipeline', 'Contacts', 'Deals', 'Proposals', 'Skills & Workflows', 'Knowledge Base', 'Goals', 'Forecasts', 'Activities'];
...
export default function Sales() {
  ...
  // Plan Calendar data
  const { data: plansData } = useApi<ExecutionPlan[]>(() => skillCalendar.getPlans({ business_area: 'sales' }), []);
  ...
  async function handleExecuteSkill(skill: SalesSkill) {
    ...
    try {
      const res = await salesSkills.executeSkill(skill.id, {});
