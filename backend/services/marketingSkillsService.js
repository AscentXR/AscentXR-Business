...
class MarketingSkillsService {
  ...
  async getSkills({ category = '', search = '', agent_id = '', page = 1, limit = 50 } = {}) {
    ...
    let sql = 'SELECT id, skill_id, name, category, description, applicable_agents, edtech_relevance, estimated_duration_minutes, output_format, tags, is_active, source_repo, created_at FROM marketing_skills';
    const params = [];
    ...
    let countSql = 'SELECT COUNT(*) FROM marketing_skills';
    const countParams = [];
  ...
  async getSkillById(id) {
    const result = await query('SELECT * FROM marketing_skills WHERE id = $1', [id]);
    return result.rows[0] || null;
  }

  async getSkillBySlug(skillId) {
    const result = await query('SELECT * FROM marketing_skills WHERE skill_id = $1', [skillId]);
    return result.rows[0] || null;
  }

  async getCategories() {
    const result = await query(
      `SELECT category, COUNT(*) as count FROM marketing_skills WHERE is_active = true GROUP BY category ORDER BY category`
    );
    return result.rows;
  }
  ...
  async getWorkflows({ category = '' } = {}) {
    let sql = `SELECT w.*,
      (SELECT COUNT(*) FROM marketing_workflow_steps WHERE workflow_id = w.id) as step_count
      FROM marketing_workflows w WHERE w.is_active = true`;
    const params = [];
  ...
  async getWorkflowById(id) {
    const workflowResult = await query('SELECT * FROM marketing_workflows WHERE id = $1', [id]);
    const workflow = workflowResult.rows[0];
    if (!workflow) return null;

    const stepsResult = await query(
      `SELECT ws.*, ms.skill_id as skill_slug, ms.name as skill_name, ms.category as skill_category,
              ms.estimated_duration_minutes as skill_duration, ms.description as skill_description
       FROM marketing_workflow_steps ws
       JOIN marketing_skills ms ON ws.skill_id = ms.id
  ...
  async startWorkflowRun(workflowId, { context = {}, created_by = 'system' } = {}) {
    ...
    const runResult = await query(
      `INSERT INTO marketing_workflow_runs (workflow_id, status, current_step, total_steps, context, created_by, started_at)
    ...
    for (const step of workflow.steps) {
      await query(
        `INSERT INTO marketing_workflow_run_steps (run_id, step_id, status) VALUES ($1, $2, 'pending')`,
        [run.id, step.id]
      );
    }
  ...
  async advanceWorkflowRun(runId) {
    ...
    if (!pendingStep) {
      // All steps completed
      await query(
        `UPDATE marketing_workflow_runs SET status = 'completed', completed_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = $1`,
        [runId]
      );
    ...
    const stepDetail = await query(
      `SELECT ws.*, ms.* FROM marketing_workflow_steps ws
       JOIN marketing_skills ms ON ws.skill_id = ms.id
       WHERE ws.id = $1`,
      [pendingStep.step_id]
    );
    ...
    await query(
      `UPDATE marketing_workflow_run_steps SET status = 'running', started_at = CURRENT_TIMESTAMP WHERE id = $1`,
      [pendingStep.id]
    );
    ...
    await query(
      `UPDATE marketing_workflow_runs SET current_step = $1, status = 'running', updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
    ...
    try {
      ...
      const agentId = step.agent_id || step.applicable_agents?.[0] || 'content-creator';
      ...
      await query(
        `UPDATE marketing_workflow_run_steps SET task_id = $1 WHERE id = $2`,
      ...
      await query(
        `UPDATE marketing_workflow_run_steps SET status = 'failed' WHERE id = $1`,
        [pendingStep.id]
      );
      await query(
        `UPDATE marketing_workflow_runs SET status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE id = $1`,
        [runId]
      );
      throw err;
    }
  ...
  async getWorkflowRun(runId) {
    const runResult = await query(
      `SELECT r.*, w.name as workflow_name, w.slug as workflow_slug
       ...
       JOIN marketing_workflows w ON r.workflow_id = w.id
       WHERE r.id = $1`,
      [runId]
    );
    const run = runResult.rows[0];
    ...
    const stepsResult = await query(
      `SELECT rs.*, ws.step_order, ms.name as skill_name, ms.skill_id as skill_slug, ms.category as skill_category
       FROM marketing_workflow_run_steps rs
       JOIN marketing_workflow_steps ws ON rs.step_id = ws.id
       JOIN marketing_skills ms ON ws.skill_id = ms.id
       WHERE rs.run_id = $1
       ORDER BY ws.step_order`,
      [runId]
  ...
  async cancelWorkflowRun(runId) {
    const result = await query(
      `UPDATE marketing_workflow_runs SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP
    ...
    await query(
      `UPDATE marketing_workflow_run_steps SET status = 'skipped' WHERE run_id = $1 AND status = 'pending'`,
      [runId]
    );
  ...
  async completeRunStep(runStepId, resultSummary) {
    await query(
      `UPDATE marketing_workflow_run_steps SET status = 'completed', result_summary = $1, completed_at = CURRENT_TIMESTAMP WHERE id = $2`,
      [resultSummary, runStepId]
    );

    // Check if all steps are completed for the run
    const runStep = await query('SELECT run_id FROM marketing_workflow_run_steps WHERE id = $1', [runStepId]);
    if (runStep.rows[0]) {
      const runId = runStep.rows[0].run_id;
      const pending = await query(
        `SELECT COUNT(*) FROM marketing_workflow_run_steps WHERE run_id = $1 AND status IN ('pending', 'running')`,
        [runId]
      );
  ...
  buildSkillPrompt(skill, context = {}) {
    ...
    sections.push(`You are executing the "${skill.name}" marketing skill for Ascent XR.`);
    ...
    sections.push('Brand Voice: Professional, innovative, student-centered, evidence-based');
...
module.exports = new MarketingSkillsService();
